...
description: 公众号改写、排版与发布：开局选择“粗稿改写并排版”或“原文锁定只排版”，再把文章制作成可复制到微信公众号的图文 HTML，按语义配图并制作 2.35:1 头条封面；用户明确要求时写入草稿箱。适用于公众号改稿、文章美化、原文保真排版、图文配图、公众号封面、copy-ready WeChat HTML、草稿箱和发布到公众号等任务。
---
...
开始处理正文前必须确定一种模式。用户未明确说明时，先让用户二选一；选择前不得改写、规划配图或制作预览：

1. **粗稿改写并排版**（`rewrite`）：允许优化标题、结构、开头、段落和表达，但保留事实边界、核心观点与证据，不编造经历或数据。
2. **原文锁定只排版**（`preserve`）：原文字符、标点和顺序必须保留。只添加 HTML 排版、图片、图注、来源说明及用户明确提供的标题或副标题；不得润色、删减、合并、调序或自动消除 AI 腔。

用户已经明确说“帮我改写”或“原文一字不改”时直接采用对应模式，不重复询问。把选择写入 `image-plan.json` 的 `content_mode`。`preserve` 模式缺少标题或副标题时先请用户补充，或取得单独授权后再新增；所有新增标题、副标题、图注和来源文字按 `references/qa-checklist.md` 标记，未标记的新正文会被阻断。
...
1. **收集输入**
   - 先确定 `rewrite` 或 `preserve`，再确认源文章、标题、副标题、作者或栏目名，以及用户提供的图片。
   ...
   - 只有 `rewrite` 模式可以推断克制的标题和副标题；`preserve` 模式必须使用原文或用户补充的内容。
   - 文章依赖时效事实时，先联网核查一手来源。

2. **按内容模式处理文章**
   - `rewrite` 模式按 `references/editorial-writing.md` 改写并保留事实、立场和证据边界，完成后向用户说明主要改动。
   - `preserve` 模式只改变 HTML 结构和视觉节奏；可以在原句边界拆段，但不得改变任何原文字符、标点或顺序。文案检查只报告问题，不执行改写。
...
7. **视觉验证**
   ...
   - 必须以约 `375-390px` 宽度打开完整预览，保存首屏和整页截图。检查通过后按 `references/publishing.md` 生成与当前正文哈希绑定的 `visual-qa.json`；自动检查只证明审查产物存在，不能替代视觉判断。
   - `publish.ts` 的正式入口只接受已完成组件排版和视觉审查的 HTML；Markdown 自动渲染只用于内部工作预览。它默认只准备正式可复制版，通常同时输出正文片段和预览，必须接收 `--visual-qa`，不处理封面，也不调用 `draft/add`。
...
8. **按需写入草稿箱**
   ...
   - 用户确认后，重新打开正式可复制预览并为已准备正文生成匹配的视觉 QA 回执，再用显式 `--create-draft` 原样复用标记正文。已有有效微信 URL 的正文图不得重复上传。
   ...
   - 使用 `--gen-cover` 时，如有语义隐喻就通过 `--cover-prompt` 传入。生图模型直接生成标题与视觉融合的成品，脚本只将其规范为 `900 x 383`；标题错字、被裁切或贴框感过重时必须重新生成。
...
- 内容模式；`rewrite` 模式说明主要改动，`preserve` 模式说明原文保留校验结果与任何额外标题、图注或来源文字。
