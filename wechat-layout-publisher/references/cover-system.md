# 微信公众号封面系统

用户要求公众号封面、写入草稿箱或完整文章包时，使用本系统。

## 目录

- 封面输出与标题策略
- 素材选择与 2.35:1 构图
- HTML 约束与导出方式
- 命名与质量检查

默认交付 `2.35:1` 公众号头条封面。`1:1` 方形封面仅在有真实发布用途时制作。

正文首图位于 H1 与副标题之后，必须是生图模型生成的 `900 x 383` 图文成品，准确标题自然融入构图。它可直接复用为公众号头条封面，也可基于同一生图方向单独生成封面，但两者都不得退化为纯图或脚本贴字。

API 边界：当前 `draft/add` 流程只通过一个 `thumb_media_id` 传入文章封面，不会把 `2.35:1` 和 `1:1` 作为两个字段上传。通过本 Skill 写草稿时，封面参数使用 `2.35:1` 头条封面。

## 封面输出

### 2.35:1 头条封面

默认规格：

- 尺寸：`900 x 383`
- 比例：`2.35:1`
- 用途：主文章封面 / 头条封面
- 优先级：本 Skill 的主要封面成品

作用：

- 展示完整或接近完整的文章标题。
- 建立文章的编辑隐喻、主体、证据或张力。
- 在公众号信息流和文章头部形成有力的第一印象。

构图规则：

- 只保留一个强视觉想法，禁止拼贴无关符号。
- 标题放在清晰安全区，通常位于左侧、左中或平衡的中央。
- 使用兆基日报配色：暖纸、墨色、低饱和砖红、鼠尾草绿和精致中性色。
- 生图模型直接生成准确标题与画面融合的成品；标题错字、多字、缺字或融合生硬时重新生成或用生图模型编辑。
- 标题必须进入构图本身的安静区域。禁止白色贴纸框、海报标签或整块黑色遮罩。
- 保持足够的视觉重心。超宽封面若只有很小的标题和图像，会显得空洞。

### 可选 1:1 方形封面

默认规格：

- 尺寸：`1080 x 1080`
- 比例：`1:1`
- 只在实际需要时制作。

适用情况：

- 多文章或多卡片组合中，次要条目需要方形缩略图。
- 平台或分享入口会把主封面裁成方形预览。
- 用户需要用于朋友圈、聊天分享、归档卡片或跨平台复用的方形社交封面。
- 封面组合需要在小尺寸下仍可阅读的紧凑缩略图。
- 用户会在本 Skill 的一键草稿 API 流程外手动使用或上传方图。

跳过情况：

- 用户只需要单篇文章主封面。
- 用户通过本 Skill 的 `draft/add` 发布，且未要求额外可复用封面。
- 标题很长，方图只会变成拥挤文字块。
- 没有任何下游入口需要方形缩略图。

方图默认风格：

- 只放短标题。
- 居中大字。
- 通常不放图片。
- 只有短标题存在歧义时才加副标题。
- 与 `2.35:1` 封面使用同一套配色。

## 标题策略

先写 `2.35:1` 头条封面的长标题，再为 `1:1` 单独压缩：

1. 找出核心动词。
2. 找出核心对象。
3. 尽量压到 4 至 10 个汉字。
4. 只保留 AI、Mac、iOS、MCP 等必要英文词。
5. 只有短标题产生歧义时才加很小的副标题。

示例：

```text
2.35:1: 开源了一个 Skill，让 AI 接管你屏幕边那张便签纸
1:1: AI 接管便签纸
```

```text
2.35:1: 第三次进山，装备比上一次轻 3.4kg
1:1: 装备减重 3.4kg
```

禁止把 2.35:1 的完整标题硬塞进方图，方形封面需要自己的短句。

## 素材选择

按语义选择生图方向：真实产品、人物、地点、页面、事件、数据或结构可作为生图参考，但官方图、用户图、截图和 SVG/HTML 都不能取代最终的生成首图。

生成封面图时遵循 `visual-quality.md`。标题要进入画面的安静区，字体、颜色、空间和视觉主体形成同一构图。

## 2.35:1 构图模式

### 编辑隐喻

- 标题位于左侧或左中。
- 生成或拍摄的主体位于右侧，并保留负空间。
- 暖纸底色，只用一条砖红强调线。

### 证据启发的生成图

- 用来源页面的真实特征建立视觉隐喻，不复刻界面、来源标志或伪证据。
- 真实截图放入正文对应论点附近，承担证据职责。

### 数据 / 结构隐喻

- 用一个生成式视觉隐喻表达关系，不伪造精确数字、时间线或图表。
- 真实数据与机制图放入正文，使用可追溯的代码可视化。

## 封面 HTML 约束

封面可制作成独立 HTML，再截图导出 PNG。与正文不同，封面 HTML 在导出前可以使用 CSS 类和 `<style>`。

设计仍需保持简单：

- 画布尺寸稳定。
- 不用装饰性光球。
- 不用随机渐变。
- 标题不压在繁忙画面上。
- 缩小预览时，全部文字仍可读。

## 导出方式

推荐轻量流程：

1. 制作独立 `cover.html`。
2. 用 Chrome/Edge 打开。
3. 使用浏览器截图能力或无头 Chrome 把封面节点导出为 PNG。

可选重型流程：

- 只有项目接受额外浏览器自动化依赖时才使用 Playwright。
- Playwright 便于自动导出 PNG 和视觉检查，但浏览器二进制可能占用数百 MB 缓存。

除非用户明确要求自动化封面渲染并接受较重安装，否则不得把 Playwright 设为必需依赖。

脚本或生图工具可以生成候选封面，但正式发布前必须把候选图裁为 `900 x 383`，人工检查准确标题、文字融合、额外文字、缩略图可读性和风格一致性。`publish.ts` 只接受已经审查通过的 `--cover` 文件，并拒绝 `--gen-cover`，防止图片在视觉 QA 之后才生成。

## 文件命名

推荐：

```text
cover/wechat-headline-cover.html
cover/wechat-headline-cover.png
cover/wechat-1x1-cover.html
cover/wechat-1x1-cover.png
cover/wechat-cover-pair-preview.html
```

普通文章发布时，把 `2.35:1` PNG 传给：

```bash
cd scripts
npx tsx publish.ts <article.html> --image-plan <image-plan.json> --source-article <source.md> \
  --title "标题" --cover <cover/wechat-headline-cover.png>
```

实际生成 `1:1` 时，将其报告为可选附加素材。除非未来明确扩展发布脚本，否则禁止暗示它已通过 `draft/add` 上传。

## 质量检查

- `2.35:1` 头条封面是主要质量闸门。
- 小尺寸缩略图下标题仍可读。
- 标题位于安静构图区，没有贴纸框或大面积深色遮罩。
- 封面标题与 HTML H1 准确一致，没有多字、缺字、乱码或额外文字。
- 视觉隐喻或来源与文章匹配。
- 生成 `1:1` 时，使用单独压缩的标题。
- 两张封面配色一致且属于同一视觉系统，方图不是盲裁。
