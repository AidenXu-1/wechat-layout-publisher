...
- `draft_copy` 已按独立流程完成“读取动力线、守住内容边界、记录声音指纹、确定修改优先级、逐项修订、定稿自检、修改说明”，没有退化成从零重写或完全冻结。
- 新建或重做的 `rewrite` 计划已记录 `editorial_contract_version: 1` 和可审查的 `editorial_plan`：核心主张、读者追问、具体开场锚点、3 至 8 个不同叙事节点、证据顺序、结尾判断、结构模式及理由都指向本次材料，没有用空泛形容词占位。`--allow-legacy-editorial` 只用于未改动的旧文章包，没有被新任务借用。
- `draft_copy` 的 `editorial_plan` 已记录 `voice_fingerprint` 和按顺序执行的 `revision_priorities`；声音指纹来自可见初稿，没有为了模仿样文添加粗口、网络梗、错别字或夸张语气。
- `messy_materials` 没有携带初稿专属的声音指纹字段；`final_copy` 没有携带任何编辑合同或改写计划。
...
- 完整文章包顺序固定为：HTML H1、克制副标题、生图模型生成的 `900×383` 图文首图、导语。首图的准确标题已自然融入构图，没有白框贴字或黑罩盖字。更换首图不得删除 HTML H1 或副标题。
- 写作前已完成内容分类。新闻/事件跟踪文章包含已截取的证据截图与来源 URL，或明确记录访问失败后的降级。
- `rewrite` 模式的开头先有人能感到的钩子或痛点，再进入专业解释；`preserve` 模式只记录问题，不改原开头。
- 证据先证明具体主张，再由正文指出读者该看哪里；作者推断与证据本身分开表达。
- 并列人物、玩法、步骤和清单优先使用显性标题；调查、亲历和故事优先使用自然换场。选择已记录为 `visible_sections`、`implicit_flow` 或 `hybrid`，理由符合本篇阅读需求。
- 每个阅读单元只有一个主旨。
- 长列表压缩成卡片、表格或 SVG 总览。
...
- 证据来源优先级为官方、原始社交来源、可信媒体、社区；已记录来源 URL、`captured_at` 和与文件一致的 `asset_sha256`。截图源文件至少 `320×120`，手机可读性仍需人工判断。
...
- 首图没有额外文字；其他生成图没有水印状瑕疵、标志、多余字母或海报文字。
...
- `rewrite` 已减少 `editorial-writing.md` 中列出的 AI 腔；`preserve` 只报告，不改写。
- 相邻叙事节点确实增加了新事实、解释、证据、风险或判断；转折句在下一段获得兑现，没有批量制造假悬念。
- `draft_copy` 保留了声音指纹中的人称、语域、节奏、情绪温度和表达特征，同时删掉只会拖慢理解的粗糙部分。
- 产品名、模型名、论文标题、日期和重要数字已核对。
- 最强句只强调一次，不在全文重复加粗。
- 结尾表达由前文证据支撑的真实结论，不重新复述全文。除非源文明确使用，否则不添加泛标题。
