...
- 留白充足；手机宽度下不能显得繁忙。
- 必须由生图模型生成，准确标题自然融入构图；没有多余文字、伪界面、水印、标志、海报边框或畸形字符。
...
- 元素更少、更干净的重新生成图。
- 文章围绕产品、公共页面、人物或事件时，把真实特征写入生图指令，并在正文后续使用真实截图或照片承担证据。

首张视觉不得使用真实照片、用户图、截图、SVG/HTML 或其他替代稿。机制图应放在读者理解事件或观点之后。无生图能力时只能停在本地工作预览，不得交付或进入草稿箱。

## 生成图提示词规则

首图是唯一必须带准确文章标题的生成图；标题必须在安静区与画面自然融合，禁止白色贴纸框、独立卡片、整块黑遮罩或粗糙盖字。其他生成图只生成画面内容，不添加标题、UI 卡片、图注、页码、徽章或海报文字。
...
首图提示词必须额外写明：精确标题全文、只出现一次、`2.35:1`、标题安全区、主体安全区、手机缩略图可读，以及禁止白框、黑罩、贴纸和额外文字。
...
除必须带准确标题的首图外，公众号正文尽量不把文章文字压在图片上。HTML H1 和副标题仍保留在首图上方，图注放在下方。
...
- 图片、SVG、大卡片、引用、表格、矩阵、步骤块和长截图都属于强视觉块；密度审查不能只数图片标签。
- 单元内容密集时，用一张图降低理解负担，再继续文字。
- 单元本身简单时，用留白改善节奏，避免装饰图。
- 相邻重视觉块、连续长截图和同义视觉重复属于硬失败。即使填写 `density_override_reason`，也要拆开或删除；只有职责明确区分并有正文过渡的证据与解释视觉才可同时保留。
- 同一组性能数据不能先卡片化再紧接柱状图；同一条步骤路径不能先步骤卡化再紧接流程图。每个主要语义只选择一种视觉表达。
- 代码图高度由真实内容决定，删除标题、流程和结论之间的空洞区域，禁止固定大画布制造留白。
- 使用约 `637px` 宽的 SVG `viewBox` 时，普通图中文字不得小于 `22` 个 SVG 单位，建议正文标签使用 `24-28`，标题使用 `30-34`；缩放到手机正文宽度后仍需达到约 `12px` 以上。
...
- 首图通过编辑首图闸门。
- 截图在手机上可读，且没有误裁。
- 首个 H2 阅读单元需要视觉时，语义锚点已经在标题后的前两段内出现。
- 没有连续长截图、相邻重视觉块或重复表达同一组数据与步骤的视觉。
- 首图标题准确且融入构图；其他生成图没有多余文字、标志、伪界面或水印瑕疵。
- 用户图片靠近对应主张。
- 相关用户视频使用有意选择且带时间戳的画面，不用随机缩略图。
- 网络图片与截图已记录来源，供完成回报使用。
- 每个 `generated_image` 记录真实提供方；Codex 记录 Image Gen。
- 图片生成不可用时，非首图的代码替代稿有明确标签并保留目标提示词；首图未生成时不进入正式交付。
- 代码视觉只用于结构解释，绝不冒充证据或第一张编辑图。
- 图注简短、客观。
- 没有纯装饰图片。
- 全文仍是一套视觉系统：配色、圆角、图注和间距一致。
- 以约 `375-390px` 宽度打开完整预览，检查首屏与整页的标题/首图顺序、截图可读性、图表空白、图片堆叠和移动端节奏。浏览器原生整页截图若重复平铺，改用分段滚动截图并稳定拼接；重复图不能作为审查证据。脚本不能替代这一步视觉判断。
